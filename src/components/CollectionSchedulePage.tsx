...
function formatTime(value?: string): string { if (!value) return "尚未执行"; const date = new Date(value); return Number.isNaN(date.getTime()) ? value : date.toLocaleString("zh-CN", { hour12: false }); }
  ...
  return <main className="settings-page collection-schedule-page">
    <section className="settings-section schedule-workspace">
      ...
      {activeTab === "collectors" ? <div className="schedule-panel" id="collector-schedule-panel" role="tabpanel" aria-labelledby="collector-schedule-tab" aria-busy={loading}><div className="schedule-list">
        {collectorPlans.length === 0 ? <div className="schedule-empty">暂无启用持续采集的采集端。</div> : collectorPlans.map((plan) => <article className="schedule-row collector-plan-row" key={plan.collectorId}><span className="schedule-row-icon"><MonitorUp size={20} /></span><div className="schedule-row-copy"><strong>{plan.fileName || plan.collectorId}</strong><small>{plan.organizationName} · {plan.includeMedia ? "包含图片和文件" : "仅文本"}</small><small>生成：{formatTime(plan.createdAt)} · 最近上传：{formatTime(plan.lastUploadAt)}</small><small>预计下次：{formatTime(plan.nextRunAt)}</small></div><div className="collector-plan-schedule">{plan.schedule.mode === "daily" ? `每天 ${plan.schedule.dailyTime}` : `每 ${plan.schedule.intervalMinutes} 分钟`}</div><span className={plan.executableAvailable ? "collector-file-state available" : "collector-file-state"}>{plan.executableAvailable ? "文件可用" : "文件缺失"}</span></article>)}
