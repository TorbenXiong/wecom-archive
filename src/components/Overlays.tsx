...
export function ExportDialog({ scope, format, dataRedaction, messageCount, mediaCount, onScopeChange, onFormatChange, onDataRedactionChange, onClose, onConfirm }: { scope: ExportScope; format: ExportFormat; dataRedaction: boolean; messageCount: number; mediaCount: number; onScopeChange: (scope: ExportScope) => void; onFormatChange: (format: ExportFormat) => void; onDataRedactionChange: (enabled: boolean) => void; onClose: () => void; onConfirm: () => Promise<void> }) {
  ...
  return <div className="modal-backdrop" role="presentation" onMouseDown={busy ? undefined : onClose}><section className="modal-card export-dialog-card" role="dialog" aria-modal="true" aria-labelledby="export-title" onMouseDown={(event) => event.stopPropagation()}><button className="modal-close" type="button" onClick={onClose} disabled={busy}><X size={18} /></button><span className={all ? "modal-icon warning" : "modal-icon"}>{all ? <AlertTriangle /> : <CheckCircle2 />}</span><h2 id="export-title">{all ? "确认导出全部档案" : "准备导出"}</h2><p>{all ? "此操作将包含全部会话记录。请确认服务端导出目录具备足够空间。" : "选择导出范围和格式后开始生成。"}</p><div className="export-dialog-options"><fieldset><legend>导出范围</legend>{([ ["current_conversation", "当前会话"], ["current_filter", "当前筛选"], ["entire_archive", "全部档案"] ] as const).map(([value, label]) => <label key={value}><input type="radio" name="export-scope" checked={scope === value} onChange={() => onScopeChange(value)} />{label}</label>)}</fieldset><fieldset><legend>导出格式</legend><div className="format-grid">{(["json", "csv", "html", "pdf"] as ExportFormat[]).map((item) => <button className={format === item ? "selected" : ""} key={item} type="button" onClick={() => onFormatChange(item)}>{item.toUpperCase()}</button>)}</div></fieldset><label className="export-data-redaction-option" title="导出前脱敏账号、密码、令牌、联系方式及疑似凭据片段；文件名和图片名保持原样。"><input type="checkbox" checked={dataRedaction} onChange={(event) => onDataRedactionChange(event.target.checked)} />数据脱敏</label></div><dl className="estimate-grid"><div><dt>预计消息</dt><dd>{messageCount.toLocaleString("zh-CN")}</dd></div><div><dt>媒体项目</dt><dd>{mediaCount.toLocaleString("zh-CN")}</dd></div><div><dt>输出</dt><dd>单个文件</dd></div><div><dt>格式</dt><dd>{format.toUpperCase()}</dd></div></dl>{error && <div className="inline-error"><AlertTriangle size={15} />{error}</div>}<div className="modal-actions"><button className="secondary-button" type="button" onClick={onClose} disabled={busy}>取消</button><button className="primary-button" type="button" onClick={submit} disabled={busy}>{busy ? "正在生成…" : "开始导出"}</button></div></section></div>;
}

export function LocalCollectionDialog() {
  return <div className="modal-backdrop local-collection-backdrop" role="presentation"><section className="modal-card local-collection-dialog" role="dialog" aria-modal="true" aria-labelledby="local-collection-title"><span className="modal-icon collecting"><LoaderCircle className="spin" /></span><h2 id="local-collection-title">正在采集本机数据</h2><p>正在只读复制并解析本机企业微信记录，完成后会自动合并到当前归档。</p><div className="collection-progress"><span /></div><small>请保持企业微信登录，采集期间无需进行其他操作。</small></section></div>;
}
