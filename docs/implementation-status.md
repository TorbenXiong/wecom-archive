...
| 企业版 | 加密配置、员工收集端生成与密文导入解密 | verified | 服务端配置页可保存告知文案、显示/复制/更新访问令牌并重新生成令牌或企业密钥；仅生成过采集端的旧密钥会保留；采集端仅输出 RSA-OAEP-SHA256 + AES-256-GCM `.wca` 密文包；服务端校验组织、密钥版本、认证标签和明文契约后幂等入库 |
| 企业版桌面宿主 | 独立窗口、本机服务生命周期、浏览器兼容入口 | verified | 桌面宿主自动启动无控制台服务并加载企业页面；关闭窗口停止其启动的服务；浏览器页面支持受控退出 |
